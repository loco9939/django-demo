...
def index(request):
    return HttpResponse("<h1>여기는 Blog 첫 페이지 입니다.</h1>")


def blog_home(request):
    blog_posts = Post.objects.all()
    context = {"blog_posts_key": blog_posts, "dummy": "나는 더미야"}
...
def blog_post_view(request, post_id):
    ...
    context = {"id": a_post.id, "title": a_post.title, "content": a_post.content}
